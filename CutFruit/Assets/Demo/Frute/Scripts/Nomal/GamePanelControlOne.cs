...
public class GamePanelControlOne : MonoBehaviour
{
    public RectTransform canvas;
    public CutLineControl leftLine;
    public CutLineControl rightLine;
    public RectTransform gamePanelone;
    public Fruit fruit;
    ...
    public GameUIControlOne gameUIControlOne;
    public int scoreNum = 0;

    KinectManager _manager;
	private Fruit[] _currentFruitArray = new Fruit[3];
	float timer = 2;// 计时器
	...
	Fruit CreatFruit()
	{
		if (lifeNum >= 1) {
			...
			go.transform.SetParent (gamePanelone);
			...
			//随机一个水果
			int type = Contant.GetOneTypeOne ();
	...
	void InitNewFruit(Fruit f, Fruit fru)
	{
		...
		f.transform.SetParent(gamePanelone);
		rt.anchoredPosition3D = Vector3.zero;
